...
return [
    ...
    | Supported Locales
    |--------------------------------------------------------------------------
    | Add or remove locales your application supports.
    | Each locale generates URL prefix routes: /uk/..., /en/...
    */
    'supportedLocales' => [
    ...
    |--------------------------------------------------------------------------
    | Requires middleware `LocaleSessionRedirect`.
    | On first visit, detects locale from the browser's Accept-Language header
    | and redirects to the matching localized URL if supported.
    | The result is stored in session until it expires.
    | If false, falls back to app.locale from config/app.php.
    */
    ...
    |--------------------------------------------------------------------------
    | When true, the default locale is omitted from the URL.
    | Example: if 'uk' is default, /uk/about redirects to /about.
    | Recommended to use with `localizationRedirect` middleware to avoid
    | duplicate content (important for SEO).
    */
    ...
    | Locales Order
    |--------------------------------------------------------------------------
    | Controls the order locales appear in a language switcher.
    */
    'localesOrder' => ['en', 'uk'],
    ...
    | Locales Mapping
    |--------------------------------------------------------------------------
    | Map custom URL segments to locale codes.
    | Example: ['de-AT' => 'at'] uses /at/ in URLs instead of /de-AT/.
    */
    'localesMapping' => [],
    ...
    | UTF-8 Suffix
    |--------------------------------------------------------------------------
    | Used for LC_TIME and LC_MONETARY locale settings.
    | Set to '' on Windows, '.utf8' on CentOS/RHEL.
    */
    ...
    | Ignored URLs
    |--------------------------------------------------------------------------
    | URLs that should not be processed by localization middleware.
    | API routes are excluded since language is handled via Accept-Language header.
    */
    'urlsIgnored' => ['/api', '/api/*'],
    ...
    |--------------------------------------------------------------------------
    | These HTTP methods bypass localization redirect logic.
    */
